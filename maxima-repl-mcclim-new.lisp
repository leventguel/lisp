...
(define-application-frame maxima-repl (standard-application-frame)
  ((system-command-reader :accessor system-command-reader
     ...
     :initform t))
  (:panes
    (interactor :interactor :display-function 'maxima-display
...
(defun maxima-display (frame stream)
  ...
  (let ((r form)
	...
	(catch 'return-from-debugger
	  ...
	  (let ((maxima::*mread-prompt* (if batch-or-demo-flag nil (maxima::main-prompt)))
	    ...
	    (tagbody
	      top
	      (accept 'maxima-expression :stream stream :prompt nil)
		...
		(progn
	      ...
	      #+nil (cond ((and (eq r eof) (boundp '*socket-connection*)
	  ...
	  #+nil (when (eq batch-or-demo-flag ':demo)
