...
(defun load-maxima ()
  (eval-when (:compile-toplevel :load-toplevel :execute)
    (cl-user::change-directory "/home/inline2/maxima-code/src/")
    (load "maxima-build.lisp")
    (maxima-load)
    (cl-user::change-directory "/home/inline2/")))
...
(defparameter *maxima-tempdir* "/home/inline2/temp/")
...
;	 (*terminal-io* input-stream))
    ...
    (with-drawing-options (input-stream :text-size :very-large :ink (make-random-color))
      ...
      (declare (special maxima::*socket-connection*))
	...
	(do ((r r)
	     ...
	     (maxima::d-tag))
	    (nil)
	  (declare (ignorable maxima::area-before maxima::area-after))      
	  (catch 'maxima::return-from-debugger
	    
	    (if (or (not (maxima::checklabel maxima::$inchar))
		      (not (maxima::checklabel maxima::$outchar)))
	      (incf maxima::$linenum))
	    
	    (setq maxima::c-tag (maxima::makelabel maxima::$inchar))
	    
	    (let ((maxima::*mread-prompt* (when batch-or-demo-flag nil (maxima::main-prompt)))
		  (maxima::eof-count 0))
	      ...
	      (tagbody
	       ...
	       (if (and
		    ...
		    (boundp 'maxima::*socket-connection*))
		   (progn
		     (setq input-stream *standard-input*)
		     (if batch-or-demo-flag
			 (return 'maxima::$done)
		       (progn
			 (setq maxima::*mread-prompt* nil)
			 (setq r (maxima::dbm-read input-stream nil eof))))))

	       (cond ((and (eq r eof) (boundp 'maxima::*socket-connection*)
			   (eq input-stream maxima::*socket-connection*))
		      (cond ((>=  (setq maxima::eof-count (+ 1 maxima::eof-count)) 10)
	...
	(unless maxima::$nolabels (set maxima::c-tag maxima::$__))
	...
	(setq *showtime*
	  (with-output-to-string (s)
		...
		(t
		  (format s " using ~:D bytes.~%" total-bytes)))) (force-output)))
	
	(unless maxima::$nolabels
	  (maxima::putprop 'maxima::$% (cons maxima::time-used 0) 'maxima::time)
	  (maxima::putprop maxima::d-tag (cons maxima::time-used  0) 'maxima::time))

	(if (eq (caar r) 'maxima::displayinput)
    ...
    ((not maxima::*alt-display2d*)
      ...
      (let ((maxima::*y-width* (or 12 (text-style-height (medium-text-style stream)
	     ...
	     (maxima::*x-width* (or 3725/256 (text-style-width (medium-text-style stream)
					       stream))))
        (let ((r
		(with-drawing-options (stream :text-size :very-large :text-face :roman :ink (make-random-color))
      ...
      (princ (untabify (with-output-to-string (s)
			 (cond
			   ((eq (caar object) 'maxima::displayinput)
			     (let ((maxima::$display2d nil))
			       (with-drawing-options (s :text-size :very-large :text-face :roman :ink (make-random-color))
