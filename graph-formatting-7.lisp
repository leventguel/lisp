...
(defun graph-it (&key (root-node (find-class 'basic-sheet)) dir ori)
  ...
  (let ((fm (find-frame-manager :port (find-port)))
	      (graph-it
	       (make-application-frame 'graph-it :pretty-name "class-nodes" :frame-manager (find-frame-manager :port (find-port)) :width 1000 :height 800 :root-node root-node :dir dir :ori ori)))
    (labels ((run () 
...
(define-graph-it-command (com-home :name "forward" :keystroke :home) 
    () (drei-commands::beginning-of-buffer (bounding-rectangle-position (pane-viewport-region (frame-panes *application-frame*)))))

(define-graph-it-command (com-end :name "forward" :keystroke :end) 
    () (drei-commands::end-of-buffer (bounding-rectangle-size (pane-viewport-region (frame-panes *application-frame*)))))

(define-graph-it-command (com-exit :name "quit" :keystroke (#\q :control)) 
    () (frame-exit *application-frame*))

;; this one you can use like (run-graph-it :root-node 'clim:dialog) or
;; like (run-graph-it :root-node 'drei:view)...etc.
