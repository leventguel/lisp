...
(defparameter *socket-connection* (get-frame-pane *application-frame* 'interactor))
  ...
  (let ((*debug-io* (if *listener-use-debug-io* *debug-io*
	...
	(restart-case (call-next-method)
	  (return-to-listener ()
	    ...
	    (throw 'return-to-listener *debug-io*)))))))
  ...
  (cond
    ((not maxima::*alt-display2d*)
      ...
      (let ((stream (get-frame-pane *application-frame* 'interactor))
	     ...
	     (maxima::*y-width* (or 12 (text-style-height (medium-text-style stream)
					 stream)))
	     (maxima::*x-width* (or 3725/256 (text-style-width (medium-text-style stream)
					       stream))))
        (let ((r
		(with-drawing-options (stream :text-size :huge :text-face :bold)
      ...
      (princ (with-output-to-string (s)
			 (cond
			   ((eq (caar object) 'maxima::displayinput)
			     (let (())
			       (with-drawing-options (s :text-size :huge :text-face :bold)
				 (maxima::displa `((maxima::mlabel) ,maxima::d-tag ,@(caddr object))))))
			   ((eq (car object) :lisp)
			     (with-drawing-options (s :text-sie :huge :text-face :bold)
			       (maxima::displa `((list (cadr object))))))
			   (t
			   (with-drawing-options (s :text-size :huge :text-face :bold)
...
(define-presentation-method accept ((type maxima-expression)
  ...
  (let ((*standard-output* (get-frame-pane *application-frame* 'interactor)))
    (with-drawing-options (t :ink (make-random-color))
      (maxima::dbm-read stream t stream))))
...
(define-command (com-eval :menu t :command-table lisp-commands)
  ...
  (let ((r form)
    ...
    (setq maxima::accumulated-time (+ maxima::accumulated-time maxima::time-used))
    ...
    (let ((*standard-output* (get-frame-pane *application-frame* 'interactor)))
      (with-drawing-options (t :ink (make-random-color) :text-size :huge)
        (with-translation (t 200 0)
	  (present `((maxima::mlabel) ,maxima::d-tag ,(if *boxify*
