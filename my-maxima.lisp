...
(defun load-maxima ()
  (eval-when (:compile-toplevel :load-toplevel :execute)
    (cl-user::change-directory "/home/inline/source/maxima-5.43.2/src/")
    (load "maxima-build.lisp")
    (maxima-load)
    (cl-user::change-directory "/home/inline/")))
...
(defparameter *maxima-quiet* nil)
(defparameter writefilep nil)
(defparameter *maxima-tempdir* "/home/inline/temp/")
...
(setf *maxima-initmac* "/home/inline/.maxima/wxmaxima-init.mac")
...
(define-command (com-eval :menu t :command-table lisp-commands)
  ...
  (let* ((r form)
    ...
    (with-drawing-options (input-stream :text-size :very-large :ink (funcall (lambda () clim-user::my-color)))
	...
	(when (eq batch-or-demo-flag ':demo)
	  ...
	  (let (quitting)
	    (do ((char)) (nil)
	      ...
	      (case (setq char (accept 'character :stream input-stream))
		((#\page)
		  (fresh-line)
		  (princ (maxima::break-prompt))
		  (force-output))
