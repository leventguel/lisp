...
(defvar *day-of-the-week-string* '((0 . "Mon") 
                                   (1 . "Tue")
                                   (2 . "Wed")
                                   (3 . "Thu")
                                   (4 . "Fri")
...
(defun leap-year-p (year)
	(cond ((and (integerp (/ year 100)) (integerp (/ year 400)))
               t)
              ((and (not (integerp (/ year 100))) (integerp (/ year 4)))
               t)
              (t nil)))

(defun month-length (month year)
	(let ((days (cdr (assoc month *days-in-month*))))
		(when (and (eql month 2) (leap-year-p year))
                  (incf days))
          days))
                   ...
                   (clim:formatting-row (stream)
                    (dotimes (d 7)
                      (clim:formatting-cell (stream :align-x :center)
                       (write-string 
                        (day-of-the-week-string (mod (- d 7) 7)) stream))))
                   (do ((date 1)
                        (first-week t nil))
                       ((> date days-in-month))
                     (clim:formatting-row (stream)
...
(define-application-frame calendar () () 
  (:panes 
   (main :application :width 400 :height 300 :display-function 'display-main)) 
